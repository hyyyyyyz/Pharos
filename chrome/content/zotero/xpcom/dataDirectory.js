...
Zotero.DataDirectory = {
	...
	isStorageDirectory: async function (dir) {
		...
		// getDatabaseFilename() rather than a literal, so this keeps recognising a
		// Pharos data directory once the database is named after ZOTERO_CONFIG.ID.
		let sqlitePath = OS.Path.join(PathUtils.parent(dir), this.getDatabaseFilename());
