...
公开的 [GitHub Pages 页面](https://hyyyyyyz.github.io/Pharos/) 是项目宣传站。真正的产品由 React 网页客户端（或 Tauri 桌面客户端）、FastAPI 后端、SQLite 数据层以及隔离运行的 PDF 翻译工作进程共同组成。
...
| 模块 | 当前能力 |
| --- | --- |
| **文库** | 导入本地 PDF、保存书目信息、翻译论文、检索全文、管理分类、阅读原文与译文、保存高亮和高亮笔记；桌面端还可直接浏览完整的本机 Zotero 文库和 PDF。 |
...
```text
Pharos Workspace/
├── database/       SQLite 元数据与 Zotero 镜像
├── library/        论文对象、元数据与论文正文索引
├── daily/          可迁移的每日论文归档
├── conversations/  按论文保存的追加式 AI 对话日志
├── annotations/    本地批注数据
...
- 打开 PDF 后，客户端会在本机提取可选择的文字，并为这篇论文或具体 Zotero 附件建立稳定索引。扫描版 PDF 需要先做 OCR，才能形成可靠上下文。
- 配置兼容 OpenAI 接口的模型后，Pharos 会先生成可复用的论文理解档案；每轮问答再从本地全文中选择与问题最相关的片段。不同论文不会共用对话。
- 接口地址和模型设置保存在 Workspace；API Key 只进入操作系统凭据库，不会写入 JSONL、SQLite、浏览器存储、日志或 Git。
- Codex 互通可以把 Codex CLI 或桌面端历史中的可见用户/助手消息导入当前论文，也可以把 Pharos 对话交给一个真实 Codex 任务。它不会导入隐藏推理、工具输出或认证文件，也不会改写 Codex 的内部 JSONL 和数据库。

### 连接 Zotero

桌面端和网页端的 Zotero 连接用途不同。

**桌面本地连接：** 保持 Zotero 运行，并允许其他本机应用通信。Pharos 通过 Zotero 官方 loopback API 建立带版本的 SQLite 镜像，再用不暴露真实路径、支持分段读取的原生协议打开 PDF。无需购买 Zotero 云存储，也不要求把附件上传到云端。Zotero 始终是数据事实来源；Pharos 不会直接读写 `zotero.sqlite`。
...
OAuth 客户端密钥只能配置在后端，不能进入前端构建产物。未配置 OAuth 时，账户设置仍会提供手动填写 Zotero 用户编号和 API 密钥的连接方式。两种云端连接方式都只同步已经存在于 Zotero Cloud 的数据；仅本机存在的附件只能由桌面客户端读取。Local API 和云端 Provider 当前都不执行写回。
...
上图主要展示 PDF 翻译执行链路；当前仓库还包含文献探索、研究项目和带本机 Zotero 镜像的 Tauri 桌面客户端。

FastAPI 后端是 Pharos 原生科研记录的数据事实来源；Zotero 条目仍以 Zotero 为准，由桌面端建立本地镜像：

```text
React 网页客户端 / Tauri 桌面客户端
                  │
...
- **桌面客户端：** Tauri 2 复用相同前端构建产物；后端和翻译引擎仍然独立运行。
...
├── desktop/                 Tauri 2 桌面客户端、本地能力与打包配置
...
生产构建通过 `frontend/.env.desktop` 或构建进程中的 `VITE_API_BASE` 指定后端地址。发布工作流可以构建 macOS 通用安装包和 Windows MSI/NSIS 安装包，但当前安装包尚未签名。更多信息见 [`desktop/README.md`](desktop/README.md)。

在客户端中，通过“设置 → AI 对话”填写兼容 OpenAI 接口的 Base URL、模型和 API Key；通过“设置 → 数据与互通”查看或迁移 Workspace，并检查 Codex CLI 是否已经被识别。
