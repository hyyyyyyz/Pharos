...
网页端和桌面端都提供论文感知的 **AI 对话**。在桌面 PDF 阅读器中打开论文后，
右侧默认直接显示占满高度的 AI 对话主面板，而不是把对话藏在条目信息下方的小区块中；
仍可通过右侧导航切回信息、笔记和其他阅读器面板。已登录且配置模型时，Pharos 会在
第一次提问前开始理解当前 PDF。预理解严格绑定阅读器里实际打开的附件，即使同一条目
下有多个 PDF，也不会误读另一个附件。每篇论文拥有独立、持久化的会话和可复用的
中文理解档案；网页端和桌面端读取后端同一份按账户隔离的状态，在一端开始的对话可以
在另一端继续。

### 论文阅读与翻译

- **保留版式的英译中。** 系统通过 `pdf2zh-next` 调用 BabelDOC，生成纯中文 PDF 和双语 PDF，同时尽量保持原有分栏、图表、表格和数学公式的位置。桌面 PDF 阅读器顶栏在“在文件中查找”左侧提供翻译按钮，可直接选择“仅译文”或“中英对照”；文库中右键 PDF 条目仍保留相同入口。
...
- **电容笔手写笔记。** 网页阅读器支持手写笔输入：笔迹带压感、与高亮同样保存在 PDF 坐标中；默认防误触（手写笔优先，手指书写需手动开启），橡皮按整笔擦除（手写笔的擦除键在任何工具下都生效），工具激活时可双指平移页面，并支持文档级撤销/重做。墨迹与所有标注一样按用户和渲染版本隔离存储。
...
当前工作流保存的是研究者拥有和确认的科研记录。它**不会**自动运行实验、复现结果、验证主张，也不会自主写出一篇完整论文。页面级证据的首个闭环已经实现：论文完成页面文本提取后，桌面 PDF 阅读器可以把选中文本保存为按用户隔离的 Evidence；页码由服务端从 `PaperChunk` 验证，合法的单页选区保留 PDF 坐标，跨页或无法确认位置的选区安全降级为只保存引文。自动执行仍属于后续工作。
...
- 桌面 PDF 阅读器打开论文后，默认把 **AI 对话**作为占满高度的右侧主面板；
  通过右侧导航仍可切回信息、笔记和其他阅读器面板，关闭上下文面板后也可用顶栏入口
  重新打开对话。登录且已经配置模型时，系统会在第一次提问前开始理解当前 PDF。
  用户手动收起后，再切回同一标签页不会被强制展开。
...
- **保排版翻译。** 点击 PDF 阅读器顶栏“在文件中查找”左侧的翻译按钮，或在文库中右键 PDF 条目，
  选择「仅译文」或「中英对照」。BabelDOC 重建文档，图表、公式、分页全部保持原位，
...
- 标签和论文级笔记仍由桌面端的 Zotero 文库负责。网页端已经支持把 arXiv 编号或官方链接直接导入当前账户文库；
  从文献探索结果一键下载并导入仍是单独的后续流程。
...
- 桌面客户端已经存在，但正式签名、公证后的公开安装包以及原生移动端薄客户端仍属于后续工作。网页端已经可以在安卓平板上直接使用——交互件按触控尺寸放大、窄视口下详情面板改为滑出层、静态外壳可作为 PWA 安装到桌面——平板路径就是「打开网页、添加到主屏幕」，无需应用商店安装包。
