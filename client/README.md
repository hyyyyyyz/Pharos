...
- **自动更新检测** — 启动后每六小时检查一次新版。有新版本时，左侧栏账号上方会
  出现更新横幅（可立即前往下载或忽略该版本）；「设置 → Pharos → 关于 Pharos」里
  显示当前版本号，也可手动检查。安装由用户自己完成，Pharos 不会在后台静默替换
  正在运行的客户端。
- **模型后台** — 普通账号可在左侧栏配置自己的 AI 对话模型（OpenAI 兼容的
  Base URL / 模型 / Key / Temperature / 最大输出 Token）。Key 发送一次后在服务端
  加密保存，永不回传客户端；清空则回退到服务器模型。管理员在该位置看到的是
  管理员后台。
