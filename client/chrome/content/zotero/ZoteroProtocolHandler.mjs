...
// Pharos claims pharos:// at the OS level (app/mac/Contents/Info.plist) and deliberately leaves
// zotero:// to an installed Zotero, so a pharos:// link arrives here in a scheme this handler was
// never registered for and dies as an unresolvable URL.
//
// The fix is an alias, not a rename. zotero:// URIs are minted all over this codebase, stored
// inside saved notes and reports, and generated by third-party plugins we do not control; renaming
// the internal scheme would break all of that, and break it *silently*, because an unregistered
// scheme resolves to a cancelled channel rather than an error. So zotero:// stays the single
// canonical internal scheme and pharos:// is registered as a second doorway onto the same handler,
// with incoming URIs rewritten to zotero:// before anything inspects them.
//
// Rewriting at the door rather than teaching each extension about two schemes is what keeps this
// small: three places re-parse the literal "zotero://" prefix back out of uri.spec (DataExtension,
// PDFJSExtension, and the _extensions lookup table), and each would otherwise need its own fix.
const PHAROS_SCHEME = ZOTERO_CONFIG.ID.toLowerCase();
// Deduped because ID is "zotero" in an unrebranded build, and registering one scheme twice throws.
const HANDLED_SCHEMES = [...new Set([ZOTERO_SCHEME, PHAROS_SCHEME])];

/**
 * Rewrite a spec in any scheme this handler serves onto its canonical zotero:// form.
 *
 * @param {nsIURI|String} uriOrSpec
 * @return {String}
 */
function canonicalSpec(uriOrSpec) {
	let spec = typeof uriOrSpec == 'string' ? uriOrSpec : uriOrSpec.spec;
	if (PHAROS_SCHEME != ZOTERO_SCHEME
			&& spec.toLowerCase().startsWith(PHAROS_SCHEME + ':')) {
		return ZOTERO_SCHEME + spec.substring(PHAROS_SCHEME.length);
	}
	return spec;
}
...
/**
 * @param {nsIURI} uri
 * @return {nsIURI} The same object when it is already canonical, so the common path allocates
 *     nothing
 */
function canonicalURI(uri) {
	let spec = canonicalSpec(uri);
	return spec == uri.spec ? uri : Services.io.newURI(spec);
}
...
/**
 * @param {String} [scheme=ZOTERO_SCHEME] The scheme this instance is registered under. Necko
 *     compares nsIProtocolHandler.scheme against the registration, so each alias needs its own
 *     instance reporting its own name.
 */
export function ZoteroProtocolHandler(scheme) {
	this.wrappedJSObject = this;
	this._scheme = scheme || ZOTERO_SCHEME;
...
ZoteroProtocolHandler.prototype = {
	get scheme() {
		return this._scheme || ZOTERO_SCHEME;
	},
	protocolFlags:
	...
	getExtension: function (uri) {
		// Keyed on zotero://, so an alias-scheme URI has to be normalized before it can match
		let uriString = canonicalSpec(uri).toLowerCase();
	...
	newChannel: function (uri, loadInfo) {
		try {
			// Everything below this line, extensions included, only ever sees zotero://
...
/**
 * Every scheme this handler answers to. The canonical one is first; the rest are aliases that
 * resolve to identical zotero:// URIs.
 *
 * @static
 * @type {String[]}
 */
ZoteroProtocolHandler.schemes = HANDLED_SCHEMES;


/**
 * Rewrite a URI or spec in any scheme this handler answers to onto its canonical zotero:// form.
 * Exposed for callers outside Necko -- the command line handler and ZoteroPane.loadURI both
 * string-match on "zotero:" and would otherwise reject the alias.
 *
 * @static
 * @param {nsIURI|String} uriOrSpec
 * @return {String}
 */
ZoteroProtocolHandler.canonicalizeSpec = canonicalSpec;


/**
 * @static
 *
 * The canonical scheme is unregistered in Zotero.reinit() for tests, but the aliases are not, so
 * we clear each scheme ourselves first -- re-registering an already-registered scheme throws, and
 * one throw here would take down the rest of Zotero.init() with it.
 */
ZoteroProtocolHandler.init = function () {
	for (let scheme of HANDLED_SCHEMES) {
		try {
			Services.io.unregisterProtocolHandler(scheme);
		}
		catch {
			// Not registered, which is the normal case on first init
		}
		
		Services.io.registerProtocolHandler(
			scheme,
			new ZoteroProtocolHandler(scheme),
