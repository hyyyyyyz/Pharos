...
if [ $BUILD_MAC == 1 ]; then
	...
	if [ $UPDATE_CHANNEL == "beta" ] || [ $UPDATE_CHANNEL == "dev" ] || [ $UPDATE_CHANNEL == "source" ]; then
		perl -pi -e "s/top\.selab\.pharos/top.selab.pharos-$UPDATE_CHANNEL/" "$CONTENTSDIR/Info.plist"
	fi
	perl -pi -e "s/\{\{VERSION\}\}/$VERSION/" "$CONTENTSDIR/Info.plist"
	...
	echo
	# Assert rather than just print. This runs under errexit, so a bare grep that
	# stopped matching would abort the build with no message at all -- which is
	# exactly what happened when the identifier was first rebranded.
	if ! grep -B 1 top.selab.pharos "$CONTENTSDIR/Info.plist"; then
		echo "ERROR: CFBundleIdentifier is not a Pharos identifier -- branding regressed" >&2
		exit 1
	fi
	echo
